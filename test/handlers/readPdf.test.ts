...
describe('read_pdf Tool Handler', () => {
     ...
     beforeEach(() => { // No longer needs to be async
         ...
         mockGetPage.mockImplementation(async (pageNum: number) => {
             ...
             if (pageNum > 0 && pageNum <= mockDocumentAPI.numPages) { // Use mockDocumentAPI directly
                  return {
                      getTextContent: vi.fn().mockResolvedValueOnce({ items: [{ str: `Mock page text ${pageNum}` }] })
                  };
             }
    ...
    it('should handle multiple sources with different options', async () => {
         ...
         const secondMockGetPage = vi.fn().mockImplementation(async (pageNum: number) => {
             if (pageNum > 0 && pageNum <= 2) { // Hardcoded numPages for second mock
                 return { getTextContent: vi.fn().mockResolvedValueOnce({ items: [{ str: `URL Mock page text ${pageNum}` }] }) };
             }
             throw new Error(`Mock getPage error: Invalid page number ${pageNum}`);
         });
         ...
         const defaultMockDocumentAPI = { numPages: 3, getMetadata: mockGetMetadata, getPage: mockGetPage };
    ...
    it('should return error if local file not found', async () => {
     ...
     it('should return error if pdfjs fails to load document', async () => {
        ...
        expect(parsedResult.results[0].error).toBe(`MCP error -32600: Failed to load PDF document. Reason: ${loadError.message}`);
    });
     ...
     it('should throw McpError for invalid input arguments (Zod error)', async () => {
        ...
        await expect(handler(args)).rejects.toThrow(/Invalid arguments: include_full_text \(Expected boolean, received string\)/);
     ...
     it('should throw McpError for invalid page specification string', async () => {
        ...
        await expect(handler(args)).rejects.toThrow(McpError);
        await expect(handler(args)).rejects.toThrow(/Invalid arguments: sources.0.pages \(Page string must contain only numbers, commas, and hyphens.\)/);
        await expect(handler(args)).rejects.toHaveProperty('code', ErrorCode.InvalidParams);
    });

     it('should throw McpError for invalid page specification array (non-positive - Zod)', async () => { // Updated description
        ...
        await expect(handler(args)).rejects.toThrow(McpError);
        await expect(handler(args)).rejects.toThrow(/Invalid arguments: sources.0.pages.1 \(Number must be greater than 0\)/);
        await expect(handler(args)).rejects.toHaveProperty('code', ErrorCode.InvalidParams);
    });
     ...
     it('should handle errors during page processing gracefully when specific pages are requested', async () => {
        ...
        mockGetPage.mockImplementation(async (pageNum: number) => {
             if (pageNum === 1) return { getTextContent: vi.fn().mockResolvedValueOnce({ items: [{ str: `Mock page text 1` }] }) };
             if (pageNum === 2) throw new Error("Failed to get page 2"); // Error for page 2
             if (pageNum === 3) return { getTextContent: vi.fn().mockResolvedValueOnce({ items: [{ str: `Mock page text 3` }] }) };
